...
import { View, Text, TouchableHighlight, Alert, Image, ActivityIndicator } from 'react-native';
...
export default function MemberDetailScreen() {
  ...
  const handleDelete = () => {
    Alert.alert('ยืนยันการลบ', `คุณต้องการลบ ${memberName || 'สมาชิกนี้'} ออกจากกลุ่มใช่หรือไม่?`, [
        ...
        <View className="bg-white rounded-[32px] border border-gray-100 p-8 items-center relative shadow-sm mb-8">
          ...
          {isTargetOwner && (
            <View className="mt-3 px-3 py-1 bg-purple-100 rounded-full">
              <Text className="text-purple-700 font-kanit text-sm font-medium">
                เจ้าของ (Owner)
              </Text>
            </View>
          )}
            ...
            <View className="bg-white rounded-[24px] border border-gray-100 overflow-hidden shadow-sm mb-6">
              ...
              >
                <View className="flex-row items-center p-5">
                  <View
                    className={`w-10 h-10 rounded-full items-center justify-center mr-3 ${selectedRole === 'EDITOR' ? 'bg-teal-100' : 'bg-gray-50'}`}
                  >
                    <MaterialIcons
                      name="edit"
                      size={22}
                      color={selectedRole === 'EDITOR' ? '#0F766E' : '#6B7280'}
                    />
                  </View>
                  <View className="flex-1">
                    <Text
                      style={{ fontSize: 16, fontWeight: '500' }}
                      className={`font-kanit ${selectedRole === 'EDITOR' ? 'text-teal-900' : 'text-gray-900'}`}
                    >
                      แก้ไขได้
                    </Text>
                  ...
                  <View
                    className={`w-6 h-6 rounded-full border-2 items-center justify-center ${selectedRole === 'EDITOR' ? 'border-teal-500 bg-teal-500' : 'border-gray-300'}`}
                  >
                    {selectedRole === 'EDITOR' && (
              ...
              >
                <View className="flex-row items-center p-5">
                  <View
                    className={`w-10 h-10 rounded-full items-center justify-center mr-3 ${selectedRole === 'VIEWER' ? 'bg-orange-100' : 'bg-gray-50'}`}
                  >
                    <MaterialIcons
                      name="visibility"
                      size={22}
                      color={selectedRole === 'VIEWER' ? '#C2410C' : '#6B7280'}
                    />
                  </View>
                  <View className="flex-1">
                    <Text
                      style={{ fontSize: 16, fontWeight: '500' }}
                      className={`font-kanit ${selectedRole === 'VIEWER' ? 'text-orange-900' : 'text-gray-900'}`}
                    >
                      ดูอย่างเดียว
                    </Text>
                    <Text style={{ fontSize: 12 }} className="font-kanit text-gray-500">
                  ...
                  <View
                    className={`w-6 h-6 rounded-full border-2 items-center justify-center ${selectedRole === 'VIEWER' ? 'border-orange-500 bg-orange-500' : 'border-gray-300'}`}
                  >
                    {selectedRole === 'VIEWER' && (
                      <MaterialIcons name="check" size={14} color="white" />
                    )}
            ...
            <View className="bg-white rounded-[24px] border border-gray-100 overflow-hidden shadow-sm">
              ...
              {selectedRole !== initialRole && (
                ...
                >
                  <View className="flex-row items-center p-5">
                    ...
                    <Text
                      style={{ fontSize: 16, fontWeight: '500' }}
                      className="font-kanit text-green-700 flex-1"
                    >
              ...
              >
                <View className="flex-row items-center p-5">
                  ...
                  <Text
                    style={{ fontSize: 16, fontWeight: '500' }}
                    className="font-kanit text-red-500 flex-1"
                  >
