...
export const ListItemSkeleton: React.FC<ListItemSkeletonProps> = ({ count = 3 }) => {
  ...
  const SkeletonItem = () => (
    <View className="bg-white rounded-2xl p-4 mb-3 flex-row items-center">
      {/* Avatar Circle */}
      <Animated.View style={animatedStyle} className="w-12 h-12 rounded-full bg-gray-200" />
      ...
      {/* Icon Placeholder */}
      <Animated.View style={animatedStyle} className="w-8 h-8 rounded-full bg-gray-200" />
    </View>
  );
