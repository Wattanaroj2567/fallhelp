...
export const StepIndicator: React.FC<StepIndicatorProps> = ({
  ...
  if (isCompleted) {
    ...
    circleClass = transparent ? 'bg-black/40 border-white/30' : 'bg-white border-gray-200';
    ...
    <View className="flex-1 items-center">
      ...
      >
        {isCompleted ? (
          ...
          <Text className={`font-kanit text-sm font-semibold ${textClass}`}>{stepNumber}</Text>
        )}
      </View>
      <Text className={`text-xs text-center font-kanit ${labelClass}`}>{label}</Text>
    </View>
  );
};
  ...
  if (transparent) {
    return (
      <View className="flex-1">
        ...
        <View className="bg-black/30 pb-4" style={{ paddingTop: insets.top }}>
          <ScreenHeader title={title} onBack={onBack} transparent={true} noSafeArea={true} />
