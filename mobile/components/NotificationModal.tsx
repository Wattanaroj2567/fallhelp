...
export const NotificationModal = ({ visible, onClose }: NotificationModalProps) => {
    ...
    const {
        ...
        enabled: visible, // Only fetch when visible
    });
    ...
    const renderItem = ({ item }: { item: Notification }) => {
        ...
        return (
            <TouchableOpacity
                ...
                activeOpacity={0.7}
                className={`flex-row items-start p-4 mb-3 rounded-2xl border shadow-sm ${item.isRead
    ...
    return (
        ...
        >
            <View className="flex-1 bg-black/40 justify-center items-center px-6">
                ...
                <View
                    style={{ height: SCREEN_HEIGHT * 0.6, width: "100%" }}
                    className="bg-white rounded-[24px] overflow-hidden shadow-xl"
                >
                    {/* Header */}
                    <View className="flex-row items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 bg-white z-10">
                        <Text className="font-kanit font-bold text-xl text-gray-900">
                            การแจ้งเตือน
                        </Text>
